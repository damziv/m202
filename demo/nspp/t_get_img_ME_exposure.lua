...
local t_get_default_img_exposure = function (id, chnn)
	...
	print('request get default_img_meexposure,ret=' .. ret, 'res='..res)

	local dec, obj = pcall(cjson.decode, res)
	
	local E = {}
	return ((obj or E).default_img_meexposure or E).compensation
end
...
local t_get_img_exposure = function (id, chnn)
	...
	local ret, res = l_sdk.request(id, to_json(req))
	print('request get img_meexposure,ret=' .. ret, 'res='..res)

	local dec, obj = pcall(cjson.decode, res)
	
	local E = {}
	return ((obj or E).img_meexposure or E).compensation
end
